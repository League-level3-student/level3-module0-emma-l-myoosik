...
/*
 * 
 * You can think of a fire work as an sparksay of sparks. Run the demo jar to see the finished product
...
public class Firework{
	...
	//1. Create an sparksay of Spark objects called sparks. Don't initialize it.
	Spark[] sparks;
	
	public boolean dead = false;
	
	public Firework() {
		//2. Initialize the Spark sparksay to contain 100 Sparks. 
		sparks = new Spark[100];
		
		System.out.println(FireworkDisplay.WIDTH);
		System.out.println(FireworkDisplay.HEIGHT);
		...
		for (int i = 0; i < sparks.length; i++) {
			sparks[i] = new Spark(1368/2, 912 - 50);
		}
	...
	public void launch() {
		...
		for (int i = 0; i < sparks.length; i++) {
			sparks[i].x = sparks[i].x;
			sparks[i].y = sparks[i].y;
		}
